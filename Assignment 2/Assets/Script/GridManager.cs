...
public class GridManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            Vector2 gridPosition = GetGridPosition(mousePosition);

            GameObject ship = GameObject.FindGameObjectWithTag("Ship");
            ship.gameObject.transform.position = gridPosition;

            gridPosition = ship.GetComponent<NavigationObject>().GetGridIndex();
            ship.GetComponent<NavigationObject>().SetGridIndex();
            grid[(int)gridPosition.y, (int)gridPosition.x].GetComponent<TileScript>().SetStatus(TileStatus.START);
        }
        ...
        if (Input.GetMouseButtonDown(1))
        {
            ...
            planet.gameObject.transform.position = gridPosition;

            gridPosition = planet.GetComponent<NavigationObject>().GetGridIndex();
            planet.GetComponent<NavigationObject>().SetGridIndex();
            grid[(int)gridPosition.y, (int)gridPosition.x].GetComponent<TileScript>().SetStatus(TileStatus.GOAL);
        }

        /*if (Input.GetKeyDown(KeyCode.R))
        {
            PathManager.Instance.path;
        }*/
        ...
        if (Input.GetKeyDown(KeyCode.F)) // Start pathfinding
        {
            ...
            {
                while (Vector3.Distance(shipIndices, planetIndices) > 0.05f)
                {
                    transform.position = Vector3.MoveTowards(shipIndices, planetIndices, 0.3f * Time.deltaTime);
                }
            }
